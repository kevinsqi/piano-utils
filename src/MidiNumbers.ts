...
const PITCH_INDEXES: { [pitchName: string]: number } = {
  C: 0,
  'C#': 1,
  Db: 1,
  D: 2,
...
function buildMidiNumberAttributesCache(): {
  [midiNumber: number]: MidiNumberAttributes;
} {
  return range(MIN_MIDI_NUMBER, MAX_MIDI_NUMBER + 1).reduce(
    (
      cache: { [midiNumber: number]: MidiNumberAttributes },
      midiNumber: number
    ) => {
...
function getAttributes(midiNumber: number): MidiNumberAttributes {
